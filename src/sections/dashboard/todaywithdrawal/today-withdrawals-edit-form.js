...
export const TodayWithdrawalEditForm = (props) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      processed_by: 'admin',
      remarks: '',
      conversion_type: customer.conversion_type || '',
    },
    validationSchema: Yup.object({
      status: Yup.string().oneOf(['Approved', 'Rejected', 'Pending']).required('Status is required'),
      conversion_type: Yup.string().oneOf(['usdt', 'bnb', 'matic', 'yuva']).required('Conversion type is required'),
          ...
          >
            ...
            >
              <Select
                ...
                value={formik.values.conversion_type}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              >
                ...
                <MenuItem value={"yuva"}>Yuva</MenuItem>
              </Select>
            </Grid>
            <Grid
              xs={12}
